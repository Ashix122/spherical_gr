...
def point_plot(store, store2, store3, x,t,typer):
    ...
    if typer == "self":
        ...
        b="4*Medium-exact"
        c="16*High-exact"
    line_real, = ax.plot(x[1:], (store[1:, 0]), color="blue", label=a)
    line_imag, = ax.plot(x[1:], (store2[1:, 0]), color="red", label=b)
    line3, = ax.plot(x[1:], (store3[1:, 0]), color="green", label=c)
    ax.set_xlim(0, 150)
...
compute_pointwise_self(dx=2.5, dt=1.0)   
#compute_norm_self(dx=1.0,dt=1.0)
